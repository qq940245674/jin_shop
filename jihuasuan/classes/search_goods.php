...
class search_goods
{
	...
	public static function find($defaultWhere = '')
	{
		...
		//(5),商品一级二级分类
		$where.= intval(IReq::get('onecate')) ? ' and go.onecate = '.intval(IReq::get('onecate')) : '';
		$where.= intval(IReq::get('in')) ? ' and go.in = '.intval(IReq::get('in')) : '';
		...
		switch($order)
		{
			...
			case "sale":
			{
				$goodsObj->order = ' go.sale '.$asc.', go.sort asc';
