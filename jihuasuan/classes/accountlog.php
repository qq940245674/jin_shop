...
class AccountLog
{
	...
	//记录商家日志
	public function writeSeller($log)
	{
		if(!isset($log['user_id']))
		{
			return array('没有设置用户id');
	...
	//商家记录
	private function logSeller($num)
	{
		if( !is_numeric($num) )
		{
			$this->error[] = "错误的操作金额:{$num}";
		...
		$member_info = $obj->query('id = '.$this->config['user_id']);
		$arr = array();
		$arr['admin_id'] = $this->admin['id'];
		$arr['user_id'] = $this->config['user_id'];
		$arr['event'] = $this->allow_event[$this->event];
		$arr['note'] = IFilter::act($this->_note,'text');
		$arr['amount'] = $num;
		$arr['amount_log'] = $member_info['0']['cash'];
		$arr['type'] = $num>=0 ? 0 : 1 ;
