...
class IModel
{
	...
	public function query1($category_id)
	{
		$sql = "SELECT img,sale,seller_id,fan_money,sell_price,name,li_goods.id FROM li_goods left join li_category_extend as c on li_goods.id = c.goods_id  WHERE is_del=0 and c.category_id = ".$category_id." ORDER BY sale DESC limit 5";
		return $this->db->query($sql);
	}
	
	public function query2($hotid)
	{
		$sql = "SELECT DISTINCT go.img,go.sell_price,go.name,go.id,go.fan_money FROM li_goods as go left join li_category_extend as c on go.id = c.goods_id left join li_commend_goods as com on go.id = com.goods_id WHERE is_del=0 and com.commend_id = ".$hotid." ORDER BY sort asc limit 6";
		return $this->db->query($sql);
	}
}
?>