...
<div class="ucenter container">
    ...
    <table class="list_table m_10" width="100%" cellpadding="0" cellspacing="0">
        ...
        <?php $query = new IQuery("member");$query->fields = "prop";$query->where = "user_id = $user_id";$items = $query->find();?>
        <?php if($items[0]['prop']!=''){?>
        <?php $ids=trim($items[0]['prop'],',')?>
        <?php }?>
        <?php if(isset($ids) && $ids!=''){?>
        <tbody>
        <?php $page=(isset($_GET['page'])&&(intval($_GET['page'])>0))?intval($_GET['page']):1;?>
        <?php $prop = new IQuery("prop");$prop->where = "id in($ids) and is_send = 1";$prop->page = "$page";$items = $prop->find(); foreach($items as $key => $item){?>
            <tr>
        ...
        <tfoot><tr><td colspan="4" class="t_l"><?php echo $prop->getPageBar();?></td></tr></tfoot>
        <?php }?>
    </table>
</div>
