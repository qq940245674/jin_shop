	...
	<div class="form_content m_10">
		...
		<div class="cont">
			...
			<tbody>
                ...
                <?php $point_log = new IQuery("point_log");$point_log->where = "user_id = $user_id and $this->c_datetime";$point_log->page = "$page";$items = $point_log->find(); foreach($items as $key => $item){?>
				<tr>
                ...
                <?php }?>
                <tr><td colspan='3'><?php echo $point_log->getPageBar();?></td></tr>
			</tbody>
		</table>
		</div>
	</div>
		...
		<div class="cont">
			<form action='<?php echo IUrl::creatUrl("/ucenter/trade_ticket");?>' method='post'>
				<table class="form_table" width="100%" cellpadding="0" cellspacing="0">
					...
					<?php $point_log = new IQuery("ticket");$point_log->page = "$page";$point_log->where = "point > 0 and start_time <= '$nowtime' and end_time > '$nowtime'";$items = $point_log->find(); foreach($items as $key => $item){?>
