...
<div class='newfenlei'>
			<ul>
				<?php $c = 0; ?>
				<?php $query = new IQuery("goods as go");$query->join = "left join category_extend as ca on go.id = ca.goods_id left join category as c on c.id = ca.category_id";$query->where = "go.name like '%$this->word%' or FIND_IN_SET('$this->word',search_words)";$query->fields = "c.name,c.id,count(*) as num";$query->group = "ca.category_id";$items = $query->find(); foreach($items as $key => $item){?>
				<?php $tmpWord=urlencode($this->word);?>
				<li class="newcategory<?php echo isset($c)?$c:"";?>"><a href="<?php echo IUrl::creatUrl("/site/search_list/word/$tmpWord/cat/$item[id]");?>"><?php echo isset($item['name'])?$item['name']:"";?>(<?php echo isset($item['num'])?$item['num']:"";?>)</a></li>
				<?php $c++; ?>
				<?php }?>
			</ul>
</div>
...
<div class="search_list main f_r" style='overflow:visible;'>
	<?php $defaultWhere = array('goods' => ' go.name like "%'.$this->word.'%" or FIND_IN_SET("'.$this->word.'",go.search_words)' , 'category_extend' => $this->cat_id)?>
	...
	<ul class="display_list clearfix m_10" style='width:1000px;margin-left:-15px;margin-top:10px;'>	
		...
		<li class="clearfix <?php echo search_goods::getListShow(IFilter::act(IReq::get('show_type')));?>">
			...
			<div class="price"><span>￥<?php echo isset($item['sell_price'])?$item['sell_price']:"";?></span><s>￥<?php echo isset($item['market_price'])?$item['market_price']:"";?></s><p>销量: <?php echo isset($item['sale'])?$item['sale']:"";?></p><br>
