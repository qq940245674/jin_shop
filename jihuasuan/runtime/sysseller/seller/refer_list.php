...
<body>
	...
	<form action="<?php echo IUrl::creatUrl("/seller/refer_del");?>" method="post" name="goodsForm">
		<table class="tablesorter" cellspacing="0">
			...
			<tbody>
				...
				<?php $referObj = new IQuery("refer as re");$referObj->join = "left join goods as go on go.id = re.goods_id left join user as u on u.id = re.user_id left join admin as a on a.id = re.admin_id left join seller as se on se.id = re.seller_id";$referObj->fields = "se.seller_name,a.admin_name,u.username,re.*,go.name";$referObj->where = "go.seller_id = $seller_id";$referObj->page = "$page";$items = $referObj->find(); foreach($items as $key => $item){?>
				<tr>
					<td><input name="id[]" type="checkbox" value="<?php echo isset($item['id'])?$item['id']:"";?>" /></td>
					<td><a href="<?php echo IUrl::creatUrl("/site/products/id/$item[goods_id]");?>" target="_blank" title="<?php echo isset($item['name'])?$item['name']:"";?>"><?php echo isset($item['name'])?$item['name']:"";?></a></td>
					...
					<td>
						<a href="<?php echo IUrl::creatUrl("/seller/refer_edit/id/$item[id]");?>"><img src="<?php echo IUrl::creatUrl("")."views/".$this->theme."/skin/".$this->skin."/images/main/icn_audio.png";?>" title="回复咨询" /></a>
						<a href="javascript:delModel({link:'<?php echo IUrl::creatUrl("/seller/refer_del/id/$item[id]");?>'})"><img src="<?php echo IUrl::creatUrl("")."views/".$this->theme."/skin/".$this->skin."/images/main/icn_del.png";?>" title="删除" /></a>
					</td>
				</tr>
				<?php }?>
			</tbody>
