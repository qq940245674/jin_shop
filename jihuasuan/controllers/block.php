...
class Block extends IController
{
    ...
    */
	...
	{
		//获得相关参数
		$order_id   = IFilter::act(IReq::get('order_id'),'int');	//order_id 订单ID
		...
		if($order_id)
		{
			...
			$payment_id = $orderRow['pay_type'];	// 10
		}
		
		$create_time = strtotime($orderRow['create_time']);	
		$now_time = strtotime(date("Y-m-d H:i:s"));
	/* 	
		if($now_time-$create_time < 24*3600*3 ){
			IError::show(403,'对不起，请三天后再确认哦..');
		} */
