...
class Site extends IController
{
	...
	function index()
	{
		...
		$row = $goods->query("is_del=0 and id is not null and sale>0 GROUP BY seller_id",'img,sale,seller_id,fan_money,sell_price,name,market_price,id','sale');
		foreach($row as $k=>$v){
			$seller_name = $seller->getObj('id = '.$v['seller_id'],'true_name,home_url');
			$seller_name = $seller_name['true_name'];
			$count = $goods->query('seller_id ='.$v['seller_id'],'sale');
			$sumsale = $goods->query('seller_id ='.$v['seller_id'],'sum(sale) as sum');
			$sumsale = $sumsale['0']['sum'];
			$count = count($count);
			$row[$k]['seller_name'] = $seller_name;
			$row[$k]['count'] = $count;
			$row[$k]['sumsale'] = $sumsale;
		}
		$this->sellerRow = $row;
		
		//热卖衣服
		$this->cloth = $goods->query1(1);
		
		//热卖鞋子
		$this->shoe = $goods->query1(2);
			
		//热卖手机
		$this->phone = $goods->query1(6);
		
		//热卖配饰
		$this->peishi = $goods->query1(4);
		
		//推荐商品
		$this->tuijian = $goods->query2(4);
		
		//热卖商品
		$this->hotsell = $goods->query2(3);
		
		//特价商品
		$this->tesell = $goods->query2(2);
	...
	function pro_list()
	{
		...
		$onecateId = IFilter::act(IReq::get('onecate'),'int');
		$inId = IFilter::act(IReq::get('in'),'int');
	...
	function products()
	{
		...
		if($goods_info['seller_id'])
		{
			...
			$grade = new IModel('goods');
			$grade = $grade->query('seller_id = '.$goods_info['seller_id'],array('grade','sale'));
			foreach($grade as $v){
				$sumGrade += $v['grade'];
				$sumSale += $v['sale'];
			}
			$goods_info['sumGrade'] = $sumGrade;
			$goods_info['sumSale'] = $sumSale;
			$goods_info['seller'] = $sellerDB->getObj('id = '.$goods_info['seller_id']);
			$fan_money =  (number_format(($goods_info['sell_price']-$goods_info['fan_money'])/$goods_info['sell_price'],2))*10;
			$goods_info['zhe'] = $fan_money;
		}
