...
class Order extends IController
{
	...
	public function order_delivery_doc()
	{
		...
		$goods_id = $order->getObj('id = '.$order_id,'goods_id');
		$goods_id = $goods_id['goods_id'];
		$g = new IModel('goods');
		$seller_id = $g->getObj('id = '.$goods_id,'seller_id');
		$seller_id = $seller_id['seller_id'];
		if(!$seller_id){
			echo '取得卖家信息出错，请联系网站管理员';
			exit;
		}
	 	Order_Class::sendDeliveryGoods($order_id,$sendgoods,$seller_id,'admin',$this->admin['admin_id']);
