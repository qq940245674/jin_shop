...
class Ucenter extends IController
{
    ...
    public function order_accept()
    {
    	...
    	if($id)
    	{
			...
			if(strlen($order_array['postscript']) !=15 || !is_numeric($order_array['postscript'])){
				$mess = '淘宝订单输入错误';
			}
    ...
    function withdraw()
    {
		...
		$userObj = new IModel('user','cart');
    	$where     = 'id = '.$user_id;
    	$userRow = $userObj->getObj($where);
		
		if(empty($userRow['true_name']) || empty($userRow['cart'])){
			$this->redirect('getcart');
		}else{
	...
	function getcart()
    {
    	...
    	$where     = 'id = '.$user_id;
    	$userRow = $userObj->getObj($where);
		if($userRow['true_name'] || $userRow['cart']){
			echo '您已填写过银行卡信息,若想修改可联系客服';
			exit;
		}
		$this->redirect('getcart');
	}
	...
	function docart()
    {
		...
		$true_name = IFilter::act( IReq::get('true_name','post') ,'string');
		$cart = IFilter::act( IReq::get('cart','post') ,'number');
		if(!is_numeric($cart)){
			echo '银行卡错误';
			exit;
		}
    ...
    function withdraw_act()
    {
    	...
    	$where     = 'id = '.$user_id;
    	$userRow = $userObj->getObj($where);
		if($userRow['true_name'] != $name){
			$data = '对不起，您的收款人姓名与预留的银行卡姓名不一致';
     ...
     */
	 function offline(){
		$urlStr  = '';
    	$user_id = intval($this->user['user_id']); 	//用户ID
		$orderid = IReq::get('orderid'); 
    	$return['postscript'] = IReq::get('postscript');
		$return['orderid'] = IReq::get('orderid');
		$return['orderon'] = IReq::get('orderon');
		$return['amount'] = IReq::get('amount');
		$return['paymentid'] = IReq::get('paymentid');
		$return['mobile'] = IReq::get('mobile');
		$return['return_url'] = IReq::get('return_url');
		$return['li_notify_url'] = IReq::get('li_notify_url');
		$return['payment_type'] = 2;	
    	$sign                 = IReq::get('sign');

    	if(stripos($return['orderon'],'recharge_') !== false)
    	{
    		IError::show(403,'淘宝线下付款方式不能用于在线充值');
    		exit;
    	}

    	if( $return['orderon'] == '' || $return['return_url'] == '' || $sign == '')
    	{
    		IError::show(403,'支付参数不正确');
    	}
    	else
    	{
    		$paymentDB  = new IModel('payment');
    		$paymentRow = $paymentDB->getObj('class_name = "offline" ');
    		$pkey       = IReq::get('orderid');
			
	    	//md5校验
			$para_filter = $this->paraFilter($return);
			$para_sort = $this->argSort($para_filter);
			$mysign = $this->buildMysign($para_sort,$orderid);
			...
			else
			{
		    	...
		    	{
		    		$orderObj = new IModel('order');
		    		$orderRow = $orderObj->getObj('order_no  = "'.IFilter::act($return['orderon']).'" and pay_status = 0');
		    		if(empty($orderRow))
		    		{
		    			IError::show(403,'订单已经被处理过，请查看订单状态');
		    			exit;
			    	...
			    	/* $is_success = $memberObj->update('user_id = '.$user_id,$pay_status);
			    	if($is_success)
			    	{
			    		$return['is_success'] = 'T';
			    	}
					...
					$urlStrMD5  = md5($responseUrl.$pkey);
