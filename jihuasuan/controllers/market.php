...
class Market extends IController
{
	...
	{	
		...
		{
			$temp   = $endDate.' 23:59:59';
			$where .= " and addtime <= '{$temp}' ";
		}

		$fanliObj = new IQuery('order_log');
		$fanliObj->where = $where;
		$fanliObj->order = 'id desc';
		$fanliObj->page  = $page;
		
		$this->fanliObj  = $fanliObj;
		$this->startDate     = $startDate;
		$this->endDate       = $endDate;
		$this->fanliList = $fanliObj->find();
		

		$this->redirect('fanli_list');
	}
